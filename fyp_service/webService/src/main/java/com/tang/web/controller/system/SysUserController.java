...
//        userService.checkUserDataScope(user.getUserId());
    ...
    {
        SysUser resUser = userService.selectUserByUserName(user.getUserName());
        if (!SecurityUtils.matchesPassword(user.getOldPassword(), resUser.getPassword())){
            return error("密码修改'" + user.getUserName() + "'失败，旧密码不正确");
        }
        user.setUserId(resUser.getUserId());
        user.setPassword(SecurityUtils.encryptPassword(user.getPassword()));
        user.setUpdateBy(resUser.getUserName());
        return toAjax(userService.resetPwd(user));
    }
    /**
     * 状态修改
